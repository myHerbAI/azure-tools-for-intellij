...
public class CognitiveServicesStartupListener implements ProjectActivity {
    ...
    public static final AzureString TRY_PLAYGROUND_MESSAGE = AzureString.format("%s model based deployments detected in your Cognitive Service accounts. " +
        "You can try your own \"%s\" in playground.", "GPT*", "ChatGPT");

    @Override
    public Object execute(@Nonnull Project project, @Nonnull Continuation<? super Unit> continuation) {
        final String tryOpenAIId = TRY_OPENAI.getId();
        final String tryPlaygroundId = "user/cognitiveservices.try_playground";
                ...
                Azure.az(AzureCognitiveServices.class).list().stream()
                    ...
                    .findFirst().ifPresent(d -> {
                        ...
                        final Action<Object> dismiss = new Action<>(Action.Id.of("user/common.never_show_again"))
                            ...
                            .withAuthRequired(false);
                        AzureMessager.getMessager().info(TRY_PLAYGROUND_MESSAGE, tryPlayGround, dismiss);
                    });
            });
        }
        return null;
